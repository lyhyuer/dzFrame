...
class dz_frame_error
{
	...
	public static function system_error($message, $show = true, $save = true, $halt = true) {
		...
		list($showtrace, $logtrace) = dz_frame_error::debug_backtrace();

		if($save) {
			$messagesave = '<b>'.$message.'</b><br><b>PHP:</b>'.$logtrace;
			dz_frame_error::write_error_log($messagesave);
		}
	
		if($show) {
			if(!defined('IN_MOBILE')) {
				dz_frame_error::show_error('system', "<li>$message</li>", $showtrace, 0);
			} else {
				dz_frame_error::mobile_show_error('system', "<li>$message</li>", $showtrace, 0);
			}
		}
	...
	public static function template_error($message, $tplname) {
		$message = lang('error', $message);
		$tplname = str_replace(DZ_FRAME_ROOT, '', $tplname);
		$message = $message.': '.$tplname;
		dz_frame_error::system_error($message);
	}

	public static function debug_backtrace() {
		$skipfunc[] = 'dz_frame_error->debug_backtrace';
		$skipfunc[] = 'dz_frame_error->db_error';
		$skipfunc[] = 'dz_frame_error->template_error';
		$skipfunc[] = 'dz_frame_error->system_error';
		...
		foreach ($debug_backtrace as $k => $error) {
			$file = str_replace(DZ_FRAME_ROOT, '', $error['file']);
	...
	public static function db_error($message, $sql) {
		...
		dz_frame_error::show_error('db', $msg, $showtrace, false);
		unset($msg, $phperror);
		...
		dz_frame_error::write_error_log($errormsg);
		exit();
	...
	public static function exception_error($exception) {
		...
		foreach ($trace as $error) {
				...
				if(!empty($error['args'])) {
					...
					foreach($error['args'] as $arg) {
						...
						if(is_array($arg)) {
							...
							$fun .= (defined('DEBUG') && DEBUG) ? $arg : '%d';
						} elseif(is_float($arg)) {
							$fun .= (defined('DEBUG') && DEBUG) ? $arg : '%f';
						} else {
							$fun .= (defined('DEBUG') && DEBUG) ? '\''.dhtmlspecialchars(substr(self::clear($arg), 0, 10)).(strlen($arg) > 10 ? ' ...' : '').'\'' : '%s';
						}
						$mark = ', ';
					}
				}
			...
			$phpmsg[] = array(
			    'file' => str_replace(array(DZ_FRAME_ROOT, '\\'), array('', '/'), $error['file']),
			    'line' => $error['line'],
...
<tr>
	<td id="bodytitle" width="100%">DZ_FRAME! $title Error </td>
</tr>
EOT;
...
EOT;
	...
	public static function write_error_log($message) {
		...
		$message = dz_frame_error::clear($message);
		$time = time();
		$file =  DZ_FRAME_ROOT.'./log/'.date("Ym").'_errorlog.php';
		$hash = md5($message);
		...
		$uri = 'Request: '.dhtmlspecialchars(dz_frame_error::clear($_SERVER['REQUEST_URI']));
